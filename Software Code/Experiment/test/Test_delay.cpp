...
#include <WiFiUDP.h>
#include <Wire.h>
#include "time.h"      // Thư viện cần thiết cho NTP và thời gian

const char* ntpServer = "pool.ntp.org";
const long  gmtOffset_sec = 0; // Offset GMT+7 cho Việt Nam
...
struct TimestampedPacket {
  uint64_t timestamp_us;   // Timestamp (micro-giây) - 8 bytes
...
// IPAddress staticIP(192, 168, 0, 101); // IP tĩnh bạn muốn cho ESP
// IPAddress gateway(192, 168, 0, 1);   // Gateway của mạng (thường là .1)
...
void setup() {  
  ...
  // Chờ cho đến khi thời gian được đồng bộ
  struct tm timeinfo;
...
void loop() {
  ...
  uint64_t timestamp_us = (uint64_t)tv.tv_sec * 1000000L + tv.tv_usec;
  ...
  delay(20); // Gửi với tần suất khoảng 50Hz
