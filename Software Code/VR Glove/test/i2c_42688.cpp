...
// #define UART_RX_PIN D5 // GPIO14
// #define UART_TX_PIN D6 // GPIO12
// #define UART_BAUD_RATE 115200

// Các byte đặc biệt để báo hiệu bắt đầu một gói tin nhị phân
const byte START_BYTE_1 = 0xAA;
const byte START_BYTE_2 = 0xBB;
...
float angle_thumb = 0.0f; // Lưu giá trị đã lọc của ngón cái
float angle_index = 0.0f; // Lưu giá trị đã lọc của ngón trỏ
float angle_middle = 0.0f;
float angle_pinky = 0.0f;
float angle_ring = 0.0f;
const float ALPHA = 0.1;  // Hệ số lọc EMA cho cảm biến uốn

// const int UART_PACKET_SIZE = sizeof(UartDataReceiver);
// byte uartDataBuffer[UART_PACKET_SIZE];
...
void loop() {
  ...
  //               String(q[2], 6) + "," + 
  //               String(q[3], 6) + "," + 
  //               String(angle_thumb, 2) + "," +
  //               String(angle_thumb, 2) + "," +
  //               String(angle_thumb, 2) + "," +
  //               String(angle_index, 2) + "," +
  //               String(angle_index, 2);
  ...
  packetToSend.thumb_angle = angle_thumb;
  packetToSend.index_angle = angle_index;
  packetToSend.middle_angle = angle_middle;
  packetToSend.ring_angle = angle_ring;
  packetToSend.pinky_angle = angle_pinky;
  
  // 4. Gửi toàn bộ struct đi dưới dạng nhị phân qua UDP
  udp.beginPacket(udpAddress, udpPortIMU);
  udp.write((byte*)&packetToSend, sizeof(packetToSend)); // Gửi 36 byte dữ liệu của struct
  udp.endPacket();

  // In ra Serial để debug
  Serial.printf("Sent UDP Packet: Quat(w=%.2f), Angles(T=%.1f, I=%.1f)\n", 
                packetToSend.w, packetToSend.thumb_angle, packetToSend.index_angle);
...
void receiveAngle() {
angle_thumb = analogRead(6);
angle_index = analogRead(5);
angle_middle = analogRead(4);
angle_ring = analogRead(3);
angle_pinky = analogRead(2);
}

// void receiveUartData() {
//   static enum { WAIT_FOR_START, RECEIVING_DATA } currentState = WAIT_FOR_START;
//   static int bytesReceived = 0;
//   const byte START_BYTE_UART = 0xA5;

//   while (MySerial.available() > 0) {
//     byte incomingByte = MySerial.read();
//     if (currentState == WAIT_FOR_START) {
//       if (incomingByte == START_BYTE_UART) {
//         bytesReceived = 0;
//         currentState = RECEIVING_DATA;
//       }
//     } else if (currentState == RECEIVING_DATA) {
//       uartDataBuffer[bytesReceived++] = incomingByte;
//       if (bytesReceived >= UART_PACKET_SIZE) {
//         UartDataReceiver receivedAngles;
//         memcpy(&receivedAngles, uartDataBuffer, UART_PACKET_SIZE);

//         // Serial.printf("[UART RX] Received Angles: thumb=%.2f, index=%.2f\n", 
//         //               receivedAngles.angle1, receivedAngles.angle2);

//         // Gán trực tiếp, không lọc vì giả định bên gửi đã lọc
//         angle_thumb = receivedAngles.angle1;
//         angle_index = receivedAngles.angle2;
